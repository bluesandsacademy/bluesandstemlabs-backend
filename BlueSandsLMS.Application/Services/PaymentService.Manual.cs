...
{
    // NOTE: this partial deliberately does NOT declare _cacheBust to avoid duplicate-field errors.
    // The other partial (PaymentService.cs) should hold any fields/ctors. We only *use* _cacheBust here.
    public sealed partial class PaymentService
    {
        public async Task<RegisterPaymentResponse> RegisterManualAsync(RegisterPaymentRequest req, ClaimsPrincipal actor)
        {
            ...
            decimal expectedPricePerStudent = 5000m;
            string? validatedPromoCode = null;

            if (!string.IsNullOrWhiteSpace(req.PromoCode))
            {
                var promoCode = req.PromoCode.Trim().ToLowerInvariant();
                if (promoCode == "abjedtech2025")
                {
                    expectedPricePerStudent = 2000m;
                    validatedPromoCode = "abjedtech2025";
                }
                else
                {
                    throw new Exception($"Invalid promo code: {req.PromoCode}");
                }
            }

            if (req.PricePerStudent != expectedPricePerStudent)
            {
                throw new Exception(
    ...
    // ✅ Wrap transactional work in the retry execution strategy
    var strategy = _db.Database.CreateExecutionStrategy();
    return await strategy.ExecuteAsync(async () =>
    {
        await using var tx = await _db.Database.BeginTransactionAsync();

        // --- Idempotency by reference (UNCHANGED) ---
        ...
        if (existing != null)
        {
            ...
            {
                // already paid → fetch active sub and return
            ...
            // mark existing as paid (UNCHANGED)
        ...
        // fetch the active subscription window (UNCHANGED)
        ...
        // optional cache-bust (UNCHANGED)
        if (userSchoolId != Guid.Empty) _cacheBust?.InvalidateSchoolAdmin(userSchoolId);
        ...
        // Manual payments → (create or extend) subscription for school or individual
